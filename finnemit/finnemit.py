...
grfuel = fuel['Savanna and Grasslands'].values  #grassland and savanna fuels
...
lctfuelin = pkg_resources.resource_filename("finnemit", "data/land-cover-gm2.csv")
...
emisin = pkg_resources.resource_filename("finnemit", "data/emission-factors.csv")
...
vegemis = emis['GenVegType'].values   # generic vegetation type --> this is ignored in model
...
area = map['area_sqkm'].values  # CW: Added March 05, 2015  -- NEED set the field
...
for j in range(ngoodfires):  # edited this to have ngoodfires instead of nfires on 02.23.2009
    #
    # ##################################################
    #   QA PROCEDURES FIRST
    # ##################################################
    ...
    if totcov[j] > 101. and totcov[j] < 240.:
        vcfcount = vcfcount + 1
    ...
    if totcov[j] < 99. and totcov[j] >= 50.:
        vcfcount = vcfcount+1
        tree[j] = tree[j]*100./totcov[j]
        herb[j] = herb[j]*100./totcov[j]
        bare[j] = bare[j]*100./totcov[j]
    ...
    if totcov[j] >= 240. or totcov[j] < 1. or bare[j] == 100 : # this also include where VCF see water (values = 253)
        allbare = allbare + 1
        if lct[j] >= 15:
            continue # Skip fires that are all bare and have no LCT vegetation
        ...
        if lct[j] >= 6 and lct[j] <= 8 or lct[j] == 11 or lct[j] == 14 :    # Assign woody savanna to the pixel
            tree[j] = 50.
            herb[j] = 50.
            bare[j] = 0.

        if lct[j] == 9 or lct[j] == 10 or lct[j] == 12 or lct[j] == 13 or lct[j] == 16 :  # Assign grassland to the pixel
            tree[j] = 20.
            herb[j] = 80.
            bare[j] = 0.
    ...
    if lct[j] >= 17 or lct[j] <= 0 or lct[j] == 15: # Added Snow/Ice on 10/20/2009
       lct0 = lct0 + 1
       continue
    ...
    if lct[j] == 9 or lct[j] == 10 or lct[j] == 11 or lct[j] == 14 or lct[j] == 16:
        genveg = 1
    ...
    # 4) Urban
    if lct[j] == 13:  #: assign genveg based on VCF cover in the pixel and reset the lct value (for emission factors)
        urbnum = urbnum + 1
        if tree[j] < 40:
            genveg = 1        # grasslands
        ...
        if tree[j] >= 60:                  # assign forest based on latitude
            if lat[j] > 50:               # 10/19/2009: Changed the latitude border to 50degrees N (from 60 before) and none in S. Hemisphere
                genveg = 5
    ...
    if lct[j] == 1:  # Evergreen Needleleaf forests (06/20/2014 Changed this)
        if lat[j] > 50.:
            genveg = 5
        else:
    ...
    if lct[j] == 3:  # deciduous Needleleaf forests -- June 20, 2014: Left LCT = 3 same as old code. ONLY Changed Evergreen needleleaf forests
        if lat[j] > 50.:
            genveg = 5
        else:
            genveg = 4   # 10/19/2009: Changed the latitude border to 50degrees N (from 60 before) and none in S. Hemisphere
...
# ####################################################
    ...
    # For Brazil from Elliott Campbell, 06/14/2010 - specific to sugar case
        if (lon[j] <= -47.323 and lon[j] >= -49.156) and (lat[j] <= -20.356 and lat[j] >= -22.708):
            bmass1 = 1100.
    ...
    if genveg == 0:
        print('Fire number:',j,' removed. Something is WRONG with generic vegetation. genveg = 0')
        genveg0 = genveg0 + 1
    ...
    if bmass1 == -1:
        ...
        print('    genveg =', genveg, ' and globreg = ', globreg[j],
              ' and reg = ', reg)
        bmass0 = bmass0 + 1
...
# Reassigned emission factors based on LCT, not genveg for new emission factor table
    ...
    if lct[j] == 16:
        index = 13
    if genveg == 6:
        index = 14 # Added this on 06/20/2014 to account for temperate evergreen forests
...
print(
    ' '
    'The Input file was: ' + infile
    'The Output file was: ' + outfile
    ' '
    'The SCENARIO in this run was: ' + scename
    ' '
    'The emissions file was: ' + emisin
    ' The Fuel loading file was' + Fuelin
    'The total number of fires input was:' + numorig
    ''#'the total number of fires in the tropics was: ' + numadd
    'The number of fires processed (ngoodfires):' + ngoodfires
    ''
    'The number of urban fires: ' + urbnum
    'The number of fires removed for overlap:' + overlapct
    ' The number of fires skipped due to lct<= 0 or lct > 17:' + lct0
    ' The number of fires skipped due to Global Region = Antarctica:' + antarc
    ' The number of fires skipped due to 100% bare cover:' + allbare
    ' The number of fires skipped due to problems with genveg:' + genveg0
    ' The number of fires skipped due to bmass assignments:' + bmass0
    ' The number of fires scaled to 100:' + vcfcount
    ' The number of fires with vcf < 50:' + vcflt50
    'Total number of fires skipped:' + spixct+lct0+antarc+allbare+genveg0+bmass0+confnum
    ''
    # Added this section 08/24/2010
    'Global Totals (Tg) of biomass burned per vegetation type'
    'GLOBAL TOTAL (Tg) biomass burned (Tg),' + BMASStotal/1.e9
    'Total Temperate Forests (Tg),' + TOTTEMP/1.e9
    'Total Tropical Forests (Tg),' + TOTTROP/1.e9
    'Total Boreal Forests (Tg),' + TOTBOR/1.e9
    'Total Shrublands/Woody Savannah(Tg),' + TOTSHRUB/1.e9
    'Total Grasslands/Savannas (Tg),' + TOTGRAS/1.e9
    'Total Croplands (Tg),' + TOTCROP/1.e9
    ''
    'Global Totals (km2) of area per vegetation type'
    'TOTAL AREA BURNED (km2),' + AREATOTAL/1000000.
    'Total Temperate Forests (km2),' + TOTTEMParea/1000000.
    'Total Tropical Forests (km2),' + TOTTROParea/1000000.
    'Total Boreal Forests (km2),' + TOTBORarea/1000000.
    'Total Shrublands/Woody Savannah(km2),' + TOTSHRUBarea/1000000.
    'Total Grasslands/Savannas (km2),' + TOTGRASarea/1000000.
    'Total Croplands (km2),' + TOTCROParea/1000000.
    ''
